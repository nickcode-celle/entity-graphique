...
function makeEmaeLogoCenters(){
  ...
  function addBranch(count,side,offset){
    for(let i=0;i<count;i++){
      ...
      const y=-2.18+t*4.18
      const center=.54+1.23*Math.pow(1-t,1.28)-.20*Math.sin(Math.PI*t)
      const width=.24+.58*Math.pow(1-t,1.70)
  ...
  function addDot(count,cx,cy,offset){
    for(let i=0;i<count;i++){
      const k=i+1+offset,a=halton(k,2)*Math.PI*2,z0=halton(k,3)*2-1,r=.30*Math.cbrt(halton(k,5)),q=Math.sqrt(Math.max(0,1-z0*z0))
      pts.push(new THREE.Vector3(cx+r*q*Math.cos(a),cy+r*z0,r*q*Math.sin(a)))
    }
  }
  const dotEach=12,branchTotal=BODY_COUNT-dotEach*2,left=Math.floor(branchTotal/2),right=branchTotal-left
  addBranch(left,-1,0)
  addBranch(right,1,10000)
  addDot(dotEach,-.54,2.73,20000)
  addDot(dotEach,.54,2.73,30000)
