...
Map.prototype.set=function(key,value){
  ...
  if(transitionRequested&&state==='idle'&&!nativeMigrationCaught&&native){
    ...
    setTimeout(()=>launchCloneOfNaturalBead(key),450)
  }
  return result
}
  ...
  if(capturedScene&&capturedCamera&&marbles.length>=200){
    ...
    window.entityTransitionAPI={
      ...
      returnToTarget(target){
        ...
        capturedCamera.updateMatrixWorld(true)
        const ndc=new THREE.Vector3(target.x,target.y,0).unproject(capturedCamera)
        const camPos=new THREE.Vector3();capturedCamera.getWorldPosition(camPos)
        const dir=ndc.sub(camPos).normalize()
        returnFrom.copy(travelBead.position)
        returnTo.copy(camPos).addScaledVector(dir,210)
...
function launchCloneOfNaturalBead(i){
  ...
  capturedCamera.updateMatrixWorld(true)
  const camPos=new THREE.Vector3(),forward=new THREE.Vector3()
  capturedCamera.getWorldPosition(camPos);capturedCamera.getWorldDirection(forward)
  // Approach the lens on a simple continuous trajectory: no overshoot, no orbit beside the camera.
  cameraTarget.copy(camPos).addScaledVector(forward,7.4)
...
function updateTravel(now){
  ...
  if(state==='outbound'){
    const t=Math.min(1,(now-flightStart)/3600)
    const e=t*t*(3-2*t)
    travelBead.position.lerpVectors(startWorld,cameraTarget,e)
