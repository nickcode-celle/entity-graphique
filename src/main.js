...
// ENTITY — ColT copy_escape diagnostic
// ColT motion laws/weights are unchanged.
// ENTITY population: 100 marbles.
// The original 30-agent 2D initialization radius (20) is scaled by sqrt(100/30)
// so the initial surface density is preserved.
// Diagnostic isolation: at t=5 s exactly ONE marble is forced into escape-turn;
// spontaneous probabilistic turn initiation is disabled so any further turn can
// only come from ColT's copy_escape mechanism.

const DT = 0.005
const N = 100
const SOURCE_N = 30
const SOURCE_RADIUS = 20
const INIT_RADIUS = SOURCE_RADIUS * Math.sqrt(N / SOURCE_N)
...
Object.assign(label.style, {
  ...
  color: 'rgba(255,255,255,.75)', font: '12px Arial', lineHeight: '1.55',
  pointerEvents: 'none',
})
...
const rotateXY = (a, rad) => v(
  a.x*Math.cos(rad)-a.y*Math.sin(rad),
  a.x*Math.sin(rad)+a.y*Math.cos(rad),
  0,
)
const gauss = () => {
  let u=0,w=0
...
for (let i=0;i<N;i++) {
  ...
  starlings.push({
    pos:v(Math.random()*INIT_RADIUS, Math.random()*INIT_RADIUS, 0),
    dir:v(Math.cos(angle), Math.sin(angle), 0),
    speed:10, accel:v(), steering:v(),
    state:0, stateExit:Infinity, nextUpdate:Math.random(),
    stressOfs:0.05 + gauss()*0.02, stress:0,
    copyDuration:0, copyState:0, homePos:v(),
    mesh:makeBody(starMat.clone()),
  })
}
...
let maxTurning=0
let propagationCompleteAt=null
let firstCopiedAt=null
const everTurned = new Set()

function sortedStarNeighbours(i) {
  const s=starlings[i]
  const arr=[]
  for(let j=0;j<N;j++) if(j!==i) {
    arr.push({idx:j,d2:s.pos.distanceToSquared(starlings[j].pos)})
  }
...
function components() {
  ...
  for(let r=0;r<N;r++) if(!seen[r]) {
    ...
    while(q.length) {
      ...
      c.push(i)
      for(let j=0;j<N;j++) {
        if(!seen[j] && i!==j && starlings[i].pos.distanceTo(starlings[j].pos)<FLOCK_THRESHOLD) {
          seen[j]=true
          q.push(j)
        }
      }
...
function flockCentroid(indices) {
  const c=v()
  for(const i of indices)c.add(starlings[i].pos)
  return c.multiplyScalar(1/indices.length)
}
function flockVelocity(indices) {
  const m=v()
  for(const i of indices)m.addScaledVector(starlings[i].dir,starlings[i].speed)
  return m
}
...
function enterStarState(i,state,t) {
  ...
  if(state===2) {
    everTurned.add(i)
    if(i!==FORCED_TURN_INDEX && firstCopiedAt===null) firstCopiedAt=t
    const f=flockOf(i)
    const gc=flockCentroid(f)
    const head=safeNorm(flockVelocity(f),s.dir.clone())
...
function chooseStarState(i,t) {
  ...
  if(!ISOLATE_COPY_TEST) {
    ...
    next=Math.random()<pTurn?2:0
  }

  // Published ColT copy_escape decision has priority over the stochastic transition.
...
function starSocial(i, includeCopy) {
  ...
  const ad=v()
  for(const ni of align)ad.add(starlings[ni.idx].dir)
  if(ad.lengthSq()>1e-12)s.steering.add(ad.normalize().multiplyScalar(STAR.alignW))

  const coh=firstVisible(i,7)
  const of=v()
  for(const ni of coh)of.add(starlings[ni.idx].pos.clone().sub(s.pos))
  if(coh.length) {
  ...
  const av=v()
  for(const ni of avoid)av.add(s.pos.clone().sub(starlings[ni.idx].pos))
  if(av.lengthSq()>1e-12)s.steering.add(av.normalize().multiplyScalar(STAR.avoidW))

  if(includeCopy) {
    s.copyDuration=0
    s.copyState=0
    // ColT copy_escape checks exactly the first topo nearest neighbours,
    // then applies FOV to those candidates.
    const nearest=sortedStarNeighbours(i).slice(0,7)
    for(const ni of nearest) {
      const o=starlings[ni.idx]
      if(visible(s,o,ni.d2) && o.state===2) {
        s.copyDuration=Math.max(0,o.stateExit-simTime)
        s.copyState=2
        break
      }
    }
...
function nearestStar(pos) {
  ...
  for(let i=0;i<N;i++) {
    const d=pos.distanceToSquared(starlings[i].pos)
    if(d<b){b=d;idx=i}
  }
  return idx
}
...
function enterPredState(state,t) {
  ...
  if(state===1){
    ...
    const cs=components().sort((a,b)=>b.length-a.length)
    const target=cs[0]?.[0] ?? 0
...
function predUpdate() {
  ...
  if(p.state===0) {
    ...
    p.steering.addScaledVector(perp(p.dir),THREE.MathUtils.lerp(-1,1,Math.random()))
    const ni=nearestStar(p.pos)
    const s=starlings[ni]
    const off=s.pos.clone().sub(p.pos)
...
function updatePropagationDiagnostics() {
  const turning=starlings.filter(s=>s.state===2).length
  maxTurning=Math.max(maxTurning,turning)
  for(let i=0;i<N;i++) if(starlings[i].state===2) everTurned.add(i)
  if(propagationCompleteAt===null && everTurned.size===N) propagationCompleteAt=simTime
}
...
function physicsStep(){
  ...
  for(let i=0;i<N;i++) {
    if(starlings[i].nextUpdate<=simTime+1e-12) starUpdate(i)
  }
  if(predator.nextUpdate<=simTime+1e-12)predUpdate()

  integrateAll()
  updatePropagationDiagnostics()
  simTime+=DT
}
...
function render(){
  centroid.set(0,0,0)
  for(const s of starlings)centroid.add(s.pos)
  centroid.multiplyScalar(1/N)

  for(let i=0;i<N;i++){
    const s=starlings[i]
    s.mesh.position.copy(s.pos).sub(centroid)
    if(s.state===2) s.mesh.material.color.set(i===FORCED_TURN_INDEX ? 0xffa63d : 0xffe08a)
  ...
  const firstCopy = firstCopiedAt===null ? '—' : `${(firstCopiedAt-FORCED_TURN_AT).toFixed(2)} s`
  const propagation = propagationCompleteAt===null
    ? `${everTurned.size}/${N}`
    : `${(propagationCompleteAt-FORCED_TURN_AT).toFixed(2)} s (100/100)`

  label.textContent =
    `ColT copy_escape — 100 billes, densité 2D conservée | t=${simTime.toFixed(1)} s | ` +
    `virage=${turning} | max simultané=${maxTurning} | déjà propagé=${everTurned.size}/${N} | ` +
    `1re copie=${firstCopy} | propagation totale=${propagation} | pénalité=${penalty} | ` +
    `test=${testState} | prédateur=${predName}`
