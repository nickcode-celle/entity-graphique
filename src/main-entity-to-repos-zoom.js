...
const focus=beads[target],focusWorld=new THREE.Vector3(),reposFocus=new THREE.Vector3(),start=performance.now()/1000
...
function startRepos(anchor){if(switched)return;switched=true;reposStartedAt=performance.now()/1000;const pos=new Float64Array(N*3),vel=new Float64Array(N*3);for(let i=0;i<N;i++){const q=i*3;pos[q]=(Math.random()-.5)*240;pos[q+1]=(Math.random()-.5)*240;pos[q+2]=(Math.random()-.5)*240;vel[q]=(Math.random()-.5)*8;vel[q+1]=(Math.random()-.5)*8;vel[q+2]=(Math.random()-.5)*8}
// La bille choisie conserve exactement son identité visuelle et sa position au raccord.
...
else if(t<4.6){phase='zoom';focus.getWorldPosition(focusWorld);const u=smooth((t-2.2)/2.4),desired=focusWorld.clone().add(new THREE.Vector3(0,0,THREE.MathUtils.lerp(210,3.0,u)));camera.position.lerp(desired,.16);camera.lookAt(focusWorld)}
else if(t<4.9){phase='switch';focus.getWorldPosition(focusWorld);camera.position.copy(focusWorld).add(new THREE.Vector3(0,0,3.0));camera.lookAt(focusWorld);startRepos(focusWorld.clone())}
else {phase='repos';if(latest){for(let i=0;i<N;i++){const q=i*3;beads[i].position.set(latest[q],latest[q+1],latest[q+2]);beads[i].scale.setScalar(REPOS.TAILLE*UNIT)}}
// Pendant le dézoom, la caméra suit LA MÊME bille. Aucun choix aléatoire, aucun saut de cible.
if(latest){const tq=target*3;reposFocus.set(latest[tq],latest[tq+1],latest[tq+2]);const u=smooth((now-reposStartedAt)/3.2);const distance=THREE.MathUtils.lerp(3.0,REPOS.CAMERA,u);const revealCentre=reposFocus.clone().lerp(new THREE.Vector3(0,0,0),smooth(Math.max(0,(u-.62)/.38)));camera.position.set(revealCentre.x,revealCentre.y,revealCentre.z+distance);camera.lookAt(revealCentre)}}
overlay.textContent=phase==='entity'?'ENTITY':phase==='zoom'?`ZOOM → bille #${target}`:phase==='switch'?`RACCORD · bille #${target}`:`REPOS · dézoom bille #${target} · 40 / 1 / 60`;renderer.render(scene,camera)}
animate()
