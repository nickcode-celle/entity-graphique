...
// TEST PUR DE MURMURATION — DONUT
// Aucun code, aucune cellule, aucune rotation et aucun mouvement propre à Entity.
// On repart du moteur GPGPU de murmuration à 1000 billes, avec les curseurs
// utilisés pendant les essais historiques.
...
const BOUNDS_FACTOR=THREE.MathUtils.clamp(Number(query.get('bounds'))||.51,.02,1)
...
void main(){
  ...
  dist=length(dir);
  dir.y*=2.5;
  velocity-=normalize(dir)*delta*centralPull;
  ...
  for(float y=0.;y<height;y++){
    for(float x=0.;x<width;x++){
      ...
      if(percent<separationThresh){
        ...
        float td=alignmentThresh-separationThresh;
        float ap=(percent-separationThresh)/td;
        birdVelocity=texture2D(textureVelocity,ref).xyz;
        f=(.5-cos(ap*PI_2)*.5+.5)*delta;
        velocity+=normalize(birdVelocity)*f;
      }else{
        float td=1.-alignmentThresh;
        float ap=td==0.?1.:(percent-alignmentThresh)/td;
        f=(.5-(cos(ap*PI_2)*-.5+.5))*delta;
        velocity+=normalize(dir)*f;
      }
    }
...
const refs=new Float32Array(BIRDS*2)
const colors=new Float32Array(BIRDS*3)
const palette=[
  0xFFE600,0xFF6500,0xE5231F,0xA86A12,0x2468D8,
  0x7137C8,0x5146E5,0x28C95B,0xE95A9D,0x13BFC8
]
...
const mat=new THREE.ShaderMaterial({
  ...
  vertexShader:`
    ...
    void main(){
      vec3 c=texture2D(texturePosition,reference).xyz;
      vec3 w=c+position*marbleScale;
  ...
  fragmentShader:`
    ...
    void main(){
      ...
      float d=.42+max(dot(vNormal,l),0.)*.58;
      float z=clamp((vDepth+400.)/800.,0.,1.);
      gl_FragColor=vec4(vColor*d*mix(.72,1.,z),1.);
    }`
})
scene.add(new THREE.Mesh(geo,mat))

const controls={
  BOUNDS:BOUNDS_FACTOR,
  CENTRE:4.9,
  SEPARATION:30,
  ALIGNEMENT:8,
  COHESION:60,
...
const gui=new GUI({title:'MURMURATION — DONUT'})
gui.add(controls,'BOUNDS',.02,1,.01).name('BOUNDS').onFinishChange(value=>{
  const u=new URL(location.href)
  u.searchParams.set('bounds',value.toFixed(2))
  location.href=u.toString()
})
gui.add(controls,'CENTRE',0,30,.1).name('CENTRE').onChange(value=>{vu.centralPull.value=value})
const physics=gui.addFolder('PHYSIQUE')
...
physics.open()
gui.add(controls,'CAMERA',300,1600,10).name('CAMERA').onChange(value=>{camera.position.z=value})
...
label.textContent='1000 billes de couleur — DONUT murmuration — moteur seul'
Object.assign(label.style,{position:'fixed',left:'14px',bottom:'12px',color:'rgba(255,255,255,.60)',font:'12px Arial',pointerEvents:'none'})
...
function animate(){
  ...
  vu.predator.value.set(.5*mouseX/windowHalfX,-.5*mouseY/windowHalfY,0)
  mouseX=mouseY=10000
  gpuCompute.compute()
  su.texturePosition.value=gpuCompute.getCurrentRenderTarget(positionVariable).texture
