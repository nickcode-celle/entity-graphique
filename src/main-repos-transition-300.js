...
// Diagnostic uniquement : mesure frame, interpolation, rendu et cadence Worker. Ombres restaurées.
...
let interpolationStart=performance.now(),interpolationDuration=16.7,hasSnapshot=false,lastSnapshotAt=0
let workerIntervalMs=0,workerComputeMs=0,workerSnapshots=0
worker.onmessage=e=>{if(e.data?.type!=='snapshot')return;const now=performance.now();if(lastSnapshotAt)workerIntervalMs=now-lastSnapshotAt;lastSnapshotAt=now;workerComputeMs=e.data.computeMs||0;workerSnapshots++;const incoming=new Float64Array(e.data.positions);for(let i=0;i<renderPositions.length;i++){fromPositions[i]=renderPositions[i];toPositions[i]=incoming[i]}interpolationStart=now;interpolationDuration=Math.max(8,Math.min(80,e.data.intervalMs||16.7));hasSnapshot=true}
...
function animate(){requestAnimationFrame(animate);const frameStart=performance.now(),frameMs=frameStart-prevFrame;prevFrame=frameStart;frames++;sumFrame+=frameMs;if(frameMs>maxFrame)maxFrame=frameMs
  const i0=performance.now();renderInterpolated(frameStart);const interpMs=performance.now()-i0;sumInterp+=interpMs
