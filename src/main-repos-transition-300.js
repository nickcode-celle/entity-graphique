...
// Les 300 objets visibles d'Entity sont conservés : aucune bille de remplacement.
// Le GPGPU calcule seulement leurs nouvelles positions à partir de leurs positions exactes au déclenchement.
...
// 300 billes : seuil REPOS. Les 100 supplémentaires font déjà partie d'Entity avant le déclenchement.
...
function startRepos(){
  ...
  // Capture EXACTE des 300 positions mondiales au moment du passage.
  entityGroup.updateMatrixWorld(true)
  const startPositions=marbles.map(o=>{const p=new THREE.Vector3();o.getWorldPosition(p);return p})

  // On détache les objets réels d'Entity : le moteur Entity peut encore tourner,
  // mais ces 300 objets ne sont plus dans son graphe visible.
  scene=new THREE.Scene();scene.background=new THREE.Color(0x08090b)
  camera=new THREE.PerspectiveCamera(75,innerWidth/innerHeight,1,4000);camera.position.z=CAMERA
  scene.add(new THREE.HemisphereLight(0xffffff,0x303030,1.35))
  const key=new THREE.DirectionalLight(0xffffff,2.2);key.position.set(250,320,420);scene.add(key)
  for(let i=0;i<BIRDS;i++){
    const o=marbles[i]
    o.removeFromParent()
    o.position.copy(startPositions[i])
    o.visible=true
    scene.add(o)
  }

  // Nouveau renderer seulement pour afficher exactement ces mêmes objets.
  ...
  Object.assign(renderer.domElement.style,{position:'fixed',inset:'0',width:'100%',height:'100%',zIndex:'50'})
  ...
  vu.separationDistance={value:SEPARATION};vu.alignmentDistance={value:ALIGNEMENT};vu.cohesionDistance={value:COHESION};vu.freedomFactor={value:.75};vu.centralPull={value:CENTRE};vu.predator={value:new THREE.Vector3()}
  ...
  const params={BOUNDS:BOUNDS_FACTOR,CENTRE,SEPARATION,ALIGNEMENT,COHESION,CAMERA,VITESSE:SPEED}
  const gui=new GUI({title:'REPOS — MÊMES BILLES ENTITY'})
  for(const k of Object.keys(params))gui.add(params,k).disable()
...
function animateRepos(){
  ...
  // Lecture des positions calculées par le vrai moteur GPGPU, puis application
  // directement aux 300 objets Entity originaux. Aucune sphère de remplacement.
  const rt=gpu.getCurrentRenderTarget(pv)
...
if(label)label.textContent='ENTITY ÉLIGIBLE → REPOS · mêmes 300 billes · mêmes positions de départ · GPGPU historique'
const title=document.querySelector('.lil-gui.root > .title')
if(title)title.textContent='ENTITY → REPOS / MÊMES BILLES'
