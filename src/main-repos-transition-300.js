...
// Rendu mobile-first : les différences de couleur passent par instanceColor ;
// les géométries réellement identiques partagent maintenant un même batch.
...
const originalAdd=THREE.Object3D.prototype.add,originalRAF=window.requestAnimationFrame.bind(window)
...
const roots=new Array(BIRDS),overlayRoots=new Array(BIRDS),batchDefs=new Map(),beadEntries=Array.from({length:BIRDS},()=>[])
const startBounds=currentBounds(),startHalf=startBounds/2

const geometryKeys=new WeakMap()
function geometryKey(g){
  let k=geometryKeys.get(g);if(k)return k
  const p=g.attributes?.position,idx=g.index
  let h=2166136261>>>0
  if(p){const a=p.array;for(let i=0;i<a.length;i++){const n=Math.round(a[i]*100000);h^=n;h=Math.imul(h,16777619)>>>0}}
  if(idx){const a=idx.array;for(let i=0;i<a.length;i++){h^=a[i];h=Math.imul(h,16777619)>>>0}}
  k=`${p?.count||0}|${idx?.count||0}|${h}`;geometryKeys.set(g,k);return k
}
function materialFamily(m){
  if(Array.isArray(m))return m.map(materialFamily).join(';')
  return `${m?.type||'none'}|${m?.roughness??''}|${m?.metalness??''}|${m?.transparent?'1':'0'}|${m?.opacity??1}|${m?.side??0}|${m?.flatShading?'1':'0'}|${m?.map?.uuid||''}|${m?.alphaMap?.uuid||''}`
}
function instanceColor(m){const x=Array.isArray(m)?m[0]:m;return x?.color?.clone?.()||new THREE.Color(0xffffff)}
function batchMaterial(m){
  if(Array.isArray(m))return m.map(batchMaterial)
  const c=m.clone();if(c.color)c.color.set(0xffffff);if(c.emissive)c.emissive.set(0x000000);c.emissiveIntensity=0;c.needsUpdate=true;return c
}
function collectTemplate(src,beadIndex){
  src.updateWorldMatrix(true,true);const invRoot=new THREE.Matrix4().copy(src.matrixWorld).invert()
  src.traverse(node=>{
    if(!node.isMesh||!node.geometry||!node.material)return
    node.updateWorldMatrix(true,false);const local=new THREE.Matrix4().multiplyMatrices(invRoot,node.matrixWorld)
    const key=`${geometryKey(node.geometry)}|${materialFamily(node.material)}|${node.renderOrder}`
    let def=batchDefs.get(key)
    if(!def){def={geometry:node.geometry,material:batchMaterial(node.material),castShadow:node.castShadow,receiveShadow:node.receiveShadow,renderOrder:node.renderOrder,items:[]};batchDefs.set(key,def)}
    const item={beadIndex,local,color:instanceColor(node.material),index:-1,batch:null};def.items.push(item);beadEntries[beadIndex].push(item)
  })
}
...
for(let i=0;i<BIRDS;i++){
  ...
  initialPositions[q]=px;initialPositions[q+1]=py;initialPositions[q+2]=pz;initialVelocities[q]=vx;initialVelocities[q+1]=vy;initialVelocities[q+2]=vz;renderPositions[q]=px;renderPositions[q+1]=py;renderPositions[q+2]=pz;root.position.set(px,py,pz);root.updateMatrix();collectTemplate(src,i)
...
for(const def of batchDefs.values()){
  ...
  for(let k=0;k<def.items.length;k++){const item=def.items[k];item.index=k;item.batch=inst;tempMatrix.multiplyMatrices(roots[item.beadIndex].matrix,item.local);inst.setMatrixAt(k,tempMatrix);inst.setColorAt(k,item.color)}
  inst.instanceMatrix.needsUpdate=true;if(inst.instanceColor)inst.instanceColor.needsUpdate=true
}
function updateInstances(){
  const touched=new Set()
  for(let i=0;i<BIRDS;i++){const q=i*3,root=roots[i];root.position.set(renderPositions[q],renderPositions[q+1],renderPositions[q+2]);root.updateMatrix();overlayRoots[i].position.copy(root.position);for(const item of beadEntries[i]){tempMatrix.multiplyMatrices(root.matrix,item.local);item.batch.setMatrixAt(item.index,tempMatrix);touched.add(item.batch)}}
