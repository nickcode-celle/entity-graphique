...
// DONUT TEST : on part exactement de REPOS 2 et on ne change que l'alignement 1 -> 8.
...
const controls={BOUNDS:.51,CENTRE:4.9,SEPARATION:70,ALIGNEMENT:8,COHESION:80,CAMERA:840,TAILLE:2.2,VITESSE:.35}
...
const gui=new GUI({title:'DONUT — TEST CURSEURS'});gui.add(controls,'BOUNDS',.1,1,.01);gui.add(controls,'CENTRE',0,10,.1);gui.add(controls,'SEPARATION',0,100,1);gui.add(controls,'ALIGNEMENT',0,100,1);gui.add(controls,'COHESION',0,100,1);gui.add(controls,'CAMERA',200,1200,10).onChange(v=>camera.position.z=v);gui.add(controls,'TAILLE',.2,5,.1).onChange(v=>{const s=v/.90;for(const m of marbles)m.scale.setScalar(s)});gui.add(controls,'VITESSE',.05,2,.05)
const status=document.createElement('div');status.textContent='DONUT TEST — base REPOS 2 · alignement 8';Object.assign(status.style,{position:'fixed',left:'14px',bottom:'12px',color:'rgba(255,255,255,.62)',font:'12px Arial',pointerEvents:'none'});document.body.appendChild(status)
