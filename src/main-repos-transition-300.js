...
// Curseurs actifs pour analyser le comportement à partir des valeurs REPOS historiques.
...
const BIRDS=300,REFERENCE_BIRDS=32*32,REFERENCE_BOUNDS=800
...
const camera=new THREE.PerspectiveCamera(55,innerWidth/innerHeight,.1,3000);camera.position.z=controls.CAMERA
...
for(let i=0;i<BIRDS;i++){
  const c=cloneBody(captured[i%captured.length]);c.scale.multiplyScalar(controls.TAILLE/.90);scene.add(c);marbles.push(c)
  const p=new THREE.Vector3(Math.random()*startBounds-startHalf,Math.random()*startBounds-startHalf,Math.random()*startBounds-startHalf)
...
const gui=new GUI({title:'REPOS — TEST CURSEURS'})
gui.add(controls,'BOUNDS',.1,1,.01)
gui.add(controls,'CENTRE',0,10,.1)
gui.add(controls,'SEPARATION',0,100,1)
gui.add(controls,'ALIGNEMENT',0,100,1)
gui.add(controls,'COHESION',0,100,1)
gui.add(controls,'CAMERA',200,1200,10).onChange(v=>camera.position.z=v)
gui.add(controls,'TAILLE',.2,5,.1).onChange(v=>{const s=v/.90;for(const m of marbles)m.scale.setScalar(s)})
gui.add(controls,'VITESSE',.05,2,.05)
const status=document.createElement('div');status.textContent='REPOS — 300 billes Entity · curseurs actifs';Object.assign(status.style,{position:'fixed',left:'14px',bottom:'12px',color:'rgba(255,255,255,.62)',font:'12px Arial',pointerEvents:'none'});document.body.appendChild(status)
...
function stepRepos(dt){
  ...
  for(let i=0;i<BIRDS;i++){
    ...
    dir.copy(selfP);dir.y*=2.5;if(dir.lengthSq()>1e-9)v.addScaledVector(dir.normalize(),-d*controls.CENTRE)
    if(zone>0)for(let j=0;j<BIRDS;j++){
      ...
      if(percent<sepT){const f=(sepT/percent-1)*d;v.addScaledVector(dir.normalize(),-f)}
      else if(percent<alignT){const td=alignT-sepT,ap=td===0?1:(percent-sepT)/td;const f=(.5-Math.cos(ap*Math.PI*2)*.5+.5)*d;tmp.copy(velocities[j]);if(tmp.lengthSq()>1e-9)v.addScaledVector(tmp.normalize(),f)}
      else{const td=1-alignT,ap=td===0?1:(percent-alignT)/td;const f=(.5-(Math.cos(ap*Math.PI*2)*-.5+.5))*d;v.addScaledVector(dir.normalize(),f)}
    }
    if(v.length()>9)v.setLength(9)
  }
  for(let i=0;i<BIRDS;i++){
