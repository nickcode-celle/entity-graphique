...
// TEST : Entity eligible -> REPOS, avec séparation physique totale des deux moteurs.
const entityBodies=[]
...
// 300 billes pour rendre Entity éligible à REPOS.
const sourcePool=entityBodies.slice()
const added=[]
for(let i=0;i<100;i++){const c=cloneBody(sourcePool[Math.floor(Math.random()*sourcePool.length)]);c.position.copy(shellPoint(i));entityGroup.add(c);added.push(c)}
...
function detachToRepos(){
  detached=true
  if(controls){controls.V1=0;controls.ROTATION=0}

  // Nouveau groupe indépendant : aucune cellule, aucun travelVector, aucune animation Entity.
  reposGroup=new THREE.Group()
  const parent=entityGroup.parent
  parent.add(reposGroup)
  parent.updateMatrixWorld(true)
  entityGroup.updateMatrixWorld(true)

  state=visibleEntity.map(original=>{
    const worldPos=new THREE.Vector3()
    original.getWorldPosition(worldPos)
    const copy=cloneBody(original)
    // Le clone n'est jamais inscrit dans les tableaux/forces Entity.
    copy.userData.__travelVector=null
    copy.position.copy(reposGroup.worldToLocal(worldPos.clone()))
    reposGroup.add(copy)
    const radial=copy.position.clone().normalize()
    const tangent=new THREE.Vector3(-radial.z,radial.y*.12,radial.x).normalize()
    return {o:copy,start:copy.position.clone(),pos:copy.position.clone(),vel:tangent.multiplyScalar(rnd(2.1,3)).addScaledVector(randomUnit(),.25)}
  })

  // Les objets encore pilotés par Entity sont rendus invisibles : son RAF peut continuer,
  // mais il n'a désormais plus aucun accès aux billes visibles de REPOS.
...
function reposStep(dt,blend){
  ...
  for(let i=0;i<state.length;i++){
    ...
    for(let q=1;q<=18;q++){
      const other=state[(i+q*17)%state.length];delta.copy(other.pos).sub(s.pos);const d2=delta.lengthSq()
    ...
    if(neighbors){coh.multiplyScalar(1/neighbors).sub(s.pos);ali.multiplyScalar(1/neighbors).sub(s.vel)}else{coh.copy(avgPos).sub(s.pos);ali.copy(avgVel).sub(s.vel)}
...
if(label)label.textContent='ENTITY ÉLIGIBLE → REPOS · moteur Entity déconnecté · BOUNDS .51 · CENTRE 4.9 · SÉPARATION 30 · ALIGNEMENT 1 · COHÉSION 60 · VITESSE .35'
const title=document.querySelector('.lil-gui.root > .title')
if(title)title.textContent='ENTITY — TRANSITION REPOS / MOTEURS SÉPARÉS'
