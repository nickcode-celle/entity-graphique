...
// Test ciblé : optimisation de la recherche des voisines, sans changer la physique du DONUT.
...
function buildGrid(cellSize){grid.clear();for(let i=0;i<BIRDS;i++){const p=positions[i],x=Math.floor(p.x/cellSize),y=Math.floor(p.y/cellSize),z=Math.floor(p.z/cellSize),key=cellKey(x,y,z);let bucket=grid.get(key);if(!bucket){bucket=[];grid.set(key,bucket)}bucket.push(i)}}
function stepDonut(dt){const d=dt*controls.VITESSE,zone=controls.SEPARATION+controls.ALIGNEMENT+controls.COHESION,zone2=zone*zone,sepT=zone>0?controls.SEPARATION/zone:0,alignT=zone>0?(controls.SEPARATION+controls.ALIGNEMENT)/zone:0;if(zone>0)buildGrid(zone);for(let i=0;i<BIRDS;i++){const selfP=positions[i],selfV=velocities[i],v=nextVel[i].copy(selfV);dir.copy(selfP);dir.y*=2.5;if(dir.lengthSq()>1e-9)v.addScaledVector(dir.normalize(),-d*controls.CENTRE);if(zone>0){const cx=Math.floor(selfP.x/zone),cy=Math.floor(selfP.y/zone),cz=Math.floor(selfP.z/zone);candidates.length=0;for(let ox=-1;ox<=1;ox++)for(let oy=-1;oy<=1;oy++)for(let oz=-1;oz<=1;oz++){const bucket=grid.get(cellKey(cx+ox,cy+oy,cz+oz));if(bucket)for(let b=0;b<bucket.length;b++)candidates.push(bucket[b])}candidates.sort((a,b)=>a-b);for(let n=0;n<candidates.length;n++){const j=candidates[n];if(i===j)continue;dir.subVectors(positions[j],selfP);const dist2=dir.lengthSq();if(dist2<1e-8||dist2>zone2)continue;const percent=dist2/zone2;if(percent<sepT){const f=(sepT/percent-1)*d;v.addScaledVector(dir.normalize(),-f)}else if(percent<alignT){const td=alignT-sepT,ap=td===0?1:(percent-sepT)/td,f=(.5-Math.cos(ap*Math.PI*2)*.5+.5)*d;tmp.copy(velocities[j]);if(tmp.lengthSq()>1e-9)v.addScaledVector(tmp.normalize(),f)}else{const td=1-alignT,ap=td===0?1:(percent-alignT)/td,f=(.5-(Math.cos(ap*Math.PI*2)*-.5+.5))*d;v.addScaledVector(dir.normalize(),f)}}}if(v.length()>9)v.setLength(9)}for(let i=0;i<BIRDS;i++){velocities[i].copy(nextVel[i]);positions[i].addScaledVector(velocities[i],d*15);marbles[i].position.copy(positions[i])}}
