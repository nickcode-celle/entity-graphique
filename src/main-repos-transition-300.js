...
// Test fluidité : calcul physique déporté dans un Worker + interpolation visuelle sur le thread d'affichage.
...
const status=document.createElement('div');status.textContent=`DONUT VALIDÉ — ${donutEligible?'ÉLIGIBLE':'NON ÉLIGIBLE'} · 500 billes · 30 / 8 / 60 · WORKER + INTERPOLATION · SANS TRAÎNÉES`;Object.assign(status.style,{position:'fixed',left:'14px',bottom:'12px',color:'rgba(255,255,255,.62)',font:'12px Arial',pointerEvents:'none'});document.body.appendChild(status)
