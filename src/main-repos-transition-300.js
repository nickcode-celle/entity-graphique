...
// 500 billes minimum, REPOS acquis, 5 domaines entre 60 et 65 avec écart max 3, déclencheur émotionnel Joie.
const DONUT_ELIGIBILITY={
  beads:500,
  reposAcquired:true,
  emotionalTrigger:'JOIE',
  domains:{
    PERSONNALITE:61,
    RELATION:62,
    GOUTS:63,
...
const KNOWLEDGE_LEVEL=DONUT_ELIGIBILITY.domains.CONNAISSANCES/100,KNOWLEDGE_ON=2.5,KNOWLEDGE_OFF_MIN=5,KNOWLEDGE_OFF_MAX=9,AFTERIMAGE_LIFE=3,AFTERIMAGE_SPACING=.42,AFTERIMAGE_SIZE=9.4,AFTERIMAGE_ALPHA=.27
...
function animate(){requestAnimationFrame(animate);const now=performance.now();let dt=(now-last)/1000;if(dt>1)dt=1;last=now;if(donutEligible)stepDonut(dt);updateKnowledge(dt,now);renderer.render(scene,camera)}animate()
